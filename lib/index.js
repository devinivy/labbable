...
module.exports = class Labbable {
    ...
    ready(options, cb) {
        ...
        if (timeout) {
            ...
            // Wrap original cb in timeout-clearing callback
            cb = (err, srv) => {
            ...
            const timeoutId = setTimeout(() => {
                ...
                const ensuredCb = queue[index]; // Protect from bad index of -1, would be a flaw

                // Pluck timed-out callback out of the queue and error
                queue.splice(index, 1);
                ensuredCb(internals.timeoutError(immediate, timeout));
